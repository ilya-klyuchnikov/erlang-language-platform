...
format_error(illegal_map_exact_in_comprehension) ->
	  ...
	  bool_option(warn_missing_spec_all, nowarn_missing_spec_all,
         ...
         {match_float_zero,
          bool_option(warn_match_float_zero, nowarn_match_float_zero,
                      true, Opts)},
         {match_alias_pats,
          bool_option(warn_match_alias_pats, nowarn_match_alias_pats,
                      true, Opts)},
         {update_literal,
...
add_error(Anno, E0, #lint{gexpr_context=Context}=St) ->
    ...
    add_lint_error({Location,elp_lint,E}, File, St).
...
add_warning(Anno, W, St) ->
    {File,Location} = loc(Anno, St),
    add_lint_warning({Location,elp_lint,W}, File, St).
...
start_state(Form, St) ->
    ...
    St1 = case filename:extension(St#lint.file) of
        ".hrl" ->
            St;
        _ ->
            add_error(Anno, undefined_module, St)
    end,
    attribute_state(Form, St1#lint{state=attribute}).
...
attribute_state({attribute,A,nominal,{TypeName,TypeDef,Args}}, St) ->
    St1 = untrack_doc({type, TypeName, length(Args)}, St),
...
function_state({attribute,A,opaque,{TypeName,TypeDef,Args}}, St) ->
    St1 = untrack_doc({type, TypeName, length(Args)}, St),
    type_def(opaque, A, TypeName, TypeDef, Args, St1);
function_state({attribute,A,nominal,{TypeName,TypeDef,Args}}, St) ->
    St1 = untrack_doc({type, TypeName, length(Args)}, St),
    type_def(nominal, A, TypeName, TypeDef, Args, St1);
...
disallowed_compile_flags(Forms, St0) ->
    ...
    Errors0 =  [ {St0#lint.file,{L,elp_lint,disallowed_nowarn_bif_clash}} ||
		    {attribute,A,compile,nowarn_bif_clash} <- Forms,
                   {_,L} <- [loc(A, St0)] ],
    Errors1 = [ {St0#lint.file,{L,elp_lint,disallowed_nowarn_bif_clash}} ||
		    {attribute,A,compile,{nowarn_bif_clash, {_,_}}} <- Forms,
                   {_,L} <- [loc(A, St0)] ],
    Disabled = (not is_warn_enabled(bif_clash, St0)),
    Errors = if
		   Disabled andalso Errors0 =:= [] ->
		       [{St0#lint.file,{elp_lint,disallowed_nowarn_bif_clash}} | St0#lint.errors];
                   Disabled ->
...
check_local_opaque_types(St) ->
    ...
    FoldFun =
        fun(_Type, #typeinfo{attr = type}, AccSt) ->
                AccSt;
           (_Type, #typeinfo{attr = nominal}, AccSt) ->
                AccSt;
    ...
    case is_warn_enabled(shadow_vars, St0) of
        true ->
            foldl(fun ({V,{_,_,[A | _]}}, St) ->
                          add_error(A, {shadowed_var,V,In}, St);
                      (_, St) -> St
