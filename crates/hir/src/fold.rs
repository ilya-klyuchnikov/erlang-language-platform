...
impl<'a, T> FoldCtx<'a, T> {
    ...
    pub fn fold_record_field_body(
        ...
        if let Some(type_expr_id) = record_field_body.ty {
            ...
            &mut |acc, on, form_id: FormIdx| {
                if on == On::Entry {
                    match form_list.get(form_id) {
                        ...
                        Form::ModuleDocMetadataAttribute(_) => acc,
                        Form::DocAttribute(_) => acc,
                        Form::DocMetadataAttribute(_) => acc,
